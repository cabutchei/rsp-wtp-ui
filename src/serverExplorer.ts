...
export class ServerExplorer implements TreeDataProvider<RSPState | ServerStateNode | DeployableStateNode | ModuleStateNode> {
    ...
    private findDefaultFilePickerPath() : Uri {
        ...
        const uriToOpen : Uri = workspaceFolderUri !== undefined ? workspaceFolderUri :
    ...
    public async publish(rspId: string, server: Protocol.ServerHandle, type: number, isAsync: boolean): Promise<Protocol.Status> {
        ...
        const req: Protocol.PublishServerRequest = { server, kind : type};
    ...
    public getDefaultServerName(rspId: string, serverType: Protocol.ServerType) : string {
        ...
        let done = false;
        while (!done) {
            const found = this.RSPServersStatus.get(rspId).state.serverStates.find(state => state.server.id === needle);
            if(found !== undefined) {
    ...
    public async addLocationWizardImplementation(serverBean: Protocol.ServerBean, rspId: string,
        ...
        const defaultName: string = this.getDefaultServerName(rspId, serverType);
        const fields:  (WizardPageFieldDefinition | WizardPageSectionDefinition)[] = [];
        const nameField: WizardPageFieldDefinition = {
            id: 'id',
            type: 'textbox',
            label: 'Server Name*',
        ...
        const requiredFields: WizardPageFieldDefinition[] = [];
        const optionalFields: WizardPageFieldDefinition[] = [];
        ...
        const requiredSection: WizardPageSectionDefinition = {
            id: 'requiredSection',
        ...
        const optionalSection: WizardPageSectionDefinition = {
            id: 'optionalSection',
            ...
            pages: [
                {
                    ...
                    validator: (parameters:any) => {
                        ...
                        for(const key in req.attributes) {
                            if(parameters[key] && parameters[key] !== '') {
                                const err: string = this.validateWizardDataType(req.attributes[key].type, parameters[key]);
                                if(err !== null) {
                                    errors.push({
                                        template: { id: `${key}Validation`,
                                            content: (`${key} ${  err}`)},
                        ...
                        for(const key in opt.attributes) {
                            if(parameters[key] && parameters[key] !== '') {
                                const err: string = this.validateWizardDataType(opt.attributes[key].type, parameters[key]);
                                if(err !== null) {
                                    errors.push({
                                        template: { id: `${key}Validation`,
                                            content: (`${key} ${  err}`)},
     ...
     */
    private async getOptionalParameters(bean: Protocol.ServerBean, attributes: Record<string, unknown>): Promise<Record<string, unknown>> {
