...
- in the ```wtp-rsp-ui``` folder, run the following command: ```npm link ../rsp-wtp-client``` (assumed the two folders are at the same level in the folder hierarchy)
- the ```wtp-rsp-ui``` codebase will now use the local codebase of ```rsp-wtp-client``` instead of the one from the global npm registry.
